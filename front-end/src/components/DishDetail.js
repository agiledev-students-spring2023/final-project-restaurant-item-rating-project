  ...
  const handleDeleteReview = async (reviewId) => {
    try {
      const response = await axios.delete(
        `${serverAddress}/restaurant/${params.restaurantID}/dish/${params.dishID}/review/${reviewId}`
      );
  ...
  const handleAddReview = async () => {
    try {
      const response = await axios.get(
        `${serverAddress}/restaurant/${params.restaurantID}/dish/${params.dishID}/review?userId=${storedId}`
      );
      if (response.status === 200) {
        navigate(
  ...
  const handleFavoriteClick = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${serverAddress}/favorites/${storedId}`,
        {
          dishId: dish._id,
